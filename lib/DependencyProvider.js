...
function solveOnline(
  ...
  useTest /*: boolean */,
  extra /*: { [string]: string } */,
  cache /*: OnlineVersionsCache */
) /*: string */ {
  cache.update();
  listVersionsMemoCache.clear();
  try {
    return wasm.solve_deps(elmJson, useTest, extra, fetchElmJsonOnline, (pkg) =>
      listAvailableVersionsOnline(cache, pkg)
    );
  } catch (errorMessage) {
    throw new Error(errorMessage);
  }
...
function listAvailableVersionsOnline(
  cache /*: OnlineVersionsCache */,
  pkg /*: string */
) /*: Array<string> */ {
  ...
  const allVersionsSet = new Set(cache.getVersions(pkg));
